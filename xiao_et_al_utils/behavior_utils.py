...
def get_exp_stats(exp, get_spatial_period=False):
    ...
    vigor = (
        tail_log_df["tail_sum"]
        .rolling(int(VIGOR_WIN_SEC / tail_dt), center=True)
        .std()
        .values
    )

    # Extract bouts:
    bouts_idxs, _ = extract_segments_above_thresh(
        vigor,
        VIGOR_THR,
        min_duration=int(MIN_DURATION_S / tail_dt),
    ...
    trial_s = stim_log_df.index[
        np.ediff1d(stim_log_df["general_cl1D_base_vel"], to_begin=0) < 0
    ]
    trial_e = stim_log_df.index[
        np.ediff1d(stim_log_df["general_cl1D_base_vel"], to_begin=0) > 0
    ]
    ...
    for i in range(len(trial_df)):
        ...
        bout_idxs = np.argwhere(
            (bout_starts > trial_df.loc[i, "start"])
            & (bout_ends < trial_df.loc[i, "end"])
        )[:, 0]
        trial_df.loc[i, "bout_n"] = len(bout_idxs)

        # If there are bouts in the trial:
        if len(bout_idxs) > 0:
            trial_df.loc[i, "first_bout_latency"] = (
                bout_starts[bout_idxs[0]] - trial_df.loc[i, "start"]
            )
            trial_df.loc[i, "bout_duration"] = (
                bout_ends[bout_idxs] - bout_starts[bout_idxs]
            ).mean()

            # If we have at least 2 bouts we calculate inter-bout:
            if len(bout_idxs) > 1:
                trial_df.loc[i, "inter_bout_t"] = (
                    bout_starts[bout_idxs[1:]] - bout_ends[bout_idxs[:-1]]
                ).mean()

    if get_spatial_period:
        # Spatial periods of each trial, it it was a grating trial:
        trial_df["spatial_period"] = [
            s["grating_period"]
